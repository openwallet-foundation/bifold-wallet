...
const RecordAttribute: React.FC<RecordAttributeProps> = ({
  ...
  return (
    <View style={styles.container}>
      {attributeLabel ? attributeLabel(attribute) : <Text style={TextTheme.label}>{startCase(attribute.name)}</Text>}
            ...
            {hideAttributeValue ? (
              <TouchableOpacity activeOpacity={1} onPress={onToggleViewPressed} style={styles.link}>
                <Text style={TextTheme.normal}>
                  {shown ? t('CredentialDetails.Hide') : t('CredentialDetails.Show')}
                </Text>
              </TouchableOpacity>
            ) : null}
          </>
